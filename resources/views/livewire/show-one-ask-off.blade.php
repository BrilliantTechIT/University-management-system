        ...
        <div class="card p-2">
            <div class="row">
                <div class="col-md-6 col-12" >
                    {{-- {{$id}} --}}
                    <img src="{{ asset('users_image/' . $data->users->image) }}"
                        style="width: 250px; height: 200px; border: 50%;" alt="" srcset="">

                    <h3 class="m-3">
                        اسم صاحب الطلب: {{ $data->users->name }}
                    </h3>
                    <p>تاريخ البداء: <span class="text-success">{{ $data->fromDate }} </span>
                    <p>تاريخ الانتهاء: {{ $data->toDate }}</p>
                    <p>النوع: {{ $data->type->name??"نوع اخر" }}</p>
                    <p>الملاحظات: {{ $data->note }}</p>
                    ...
                    <form wire:submit='SaveEdite' method="post" wire:ignore.self>
                        <label class="form-lable" for="">تاريخ البداء</label>
                        <input wire:model='fromDate' type="date" class="form-control" name="" id="">

                        <label class="form-lable" for="">تاريخ الانتهاء</label>
                        <input wire:model='toDate' type="date" class="form-control" name="" id="">

                        <label class="form-lable" for="">النوع</label>
                        <select wire:model='type' class="form-control" name="" id="">
                            <option value="0">نوع اخر</option>
                            @foreach ($types as $item)
                            <option value="{{$item->id}}">{{$item->name}}</option>
                            @endforeach
                        </select>

                        <label class="form-lable" for="">الملاحظات</label>
                                ...
                                @if ($key == 0)
                                    <div class="carousel-item active">
                                        <img style="height: 500px"
                                            src="{{ asset('storage/' . $item['file']) }}" class="d-block w-100"
                                            alt="...">
                                    </div>
                                @else
                                    <div class="carousel-item">
                                        <img style="height: 500px"
                                            src="{{ asset('storage/askoff/' . $item['file']) }}" class="d-block w-100"
                                            alt="...">
                                    </div>
                                @endif
                            @endforeach
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <th scope="row">{{$key}}</th>
                                    <td>ملف</td>
                                    <td><a href="{{ asset('storage/' . $item->file) }}"  class="btn btn-primary">عرض الملف</a></td>
